...
using namespace std;
//将常用的带强度信息的点云类型pcl::PointXYZI定义为PointType
...
extern const int Horizon_SCAN = 1800; //水平方向激光点（360/0.2）
extern const float ang_res_x = 0.2; //水平方向分辨率
extern const float ang_res_y = 2.0; //垂直方向分辨率
extern const float ang_bottom = 15.0+0.1; //水平方向上的夹角
extern const int groundScanInd = 7; //扫到地面的激光线束
...
extern const bool loopClosureEnableFlag = false; //默认回环是关闭的
extern const double mappingProcessInterval = 0.3; //建图处理的时间间隔，只有大于这个值才能进行建图优化，

extern const float scanPeriod = 0.1; //雷达扫描周期
extern const int systemDelay = 0; //系统延时
extern const int imuQueLength = 200; //imu队列长度

extern const float sensorMinimumRange = 1.0; //设置传感器最小测距，通常用来过滤自身形成的点云
extern const float sensorMountAngle = 0.0; //传感器安装角度
extern const float segmentTheta = 60.0/180.0*M_PI; // decrese this value may improve accuracy（角度 = 弧度 * 180 / PI）
extern const int segmentValidPointNum = 5; //
extern const int segmentValidLineNum = 3;
extern const float segmentAlphaX = ang_res_x / 180.0 * M_PI; // 0.2 / 180 * PI
extern const float segmentAlphaY = ang_res_y / 180.0 * M_PI; // 2.0 / 180 * PI
